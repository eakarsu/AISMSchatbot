...
The supported surface is **Benefits appeal preparation** at `/api/governed-benefit-appeals`. It provides tenant- and subject-scoped cases, opaque evidence digests, deterministic assessments, optimistic state transitions, dual control, immutable event history, and connector-failure receipts.
...
The policy endpoint reports declared connectors as unconfigured. No benefits system, identity, document, SMS/voice, scheduling, partner, notification, legal, or eligibility determination integration is configured or validated.
...
2. Review `server/migrations/001_governed_benefit_appeal.sql` and apply it manually in a controlled change window:
   `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f server/migrations/001_governed_benefit_appeal.sql`
