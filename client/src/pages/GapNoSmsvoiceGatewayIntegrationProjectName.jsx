...
export default function GapNoSmsvoiceGatewayIntegrationProjectName() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-smsvoice-gateway-integration-project-name', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>No SMS/voice gateway integration (project name implies SMS but no Twilio route)</h1>
      <p style={{ color: '#666', marginBottom: 16 }}>AISMSchatbot - Gap feature</p>
