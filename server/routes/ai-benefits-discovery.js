// AI Comprehensive benefits discovery
// Intake income/family info, AI identifies all possible benefits
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISMSchatbot. Focus area: Comprehensive benefits discovery. ${`Intake income/family info, AI identifies all possible benefits`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Comprehensive benefits discovery.\n${`Intake income/family info, AI identifies all possible benefits`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'benefits-discovery', context, parsed);
    res.json({ feature: 'benefits-discovery', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[benefits-discovery]', err.message);
